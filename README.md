...
### プロジェクトについて
<!-- プロジェクトについて -->
以前制作したご褒美機能つきのTodoアプリ「Dreamer」をブラッシュアップしました。
前回の実装では、アップロードしたイメージを表示するUIがあるにも関わらずユーザー自身の写真をアップロードできないという課題がありました。そしてサーバー側の認証ロジックは最小限であり不安定な書き方でした。それに加え、テスト実装にも力を入れたいという自身の課題もあり、これらを解決できるブラッシュアップを目指しました。
結果的に、全てのフォームにServer Actionsを取り入れることでサーバー側のバリデーションを強化し、Supabase Storageを利用することでユーザーがストレージに画像をアップロードできるようにしました。前回のテストはJestによるユニットテストが主でしたが、今回単体テストにvitest、結合テストにPlayWrightを使用し、テスト内容を増やしました。それに伴いDBの変更、よりアプリの趣旨に合うUIの変更、styled-componentsからTailwind CSSへの変更などを行い、ユーザーに不安を感じにくさせず、後の変更時にも有益となるようなアプリの実装を目指しました。しかし、テストの実装によって全体的に保守性をアップするという面ではまだ課題が残っており、テスト構築方法をより考えていく必要性を感じました。
